...
def api_call(excel_file,save_path):
    ...
    try:
        ...
        print('Got Python Script Error')
        print('At Line Number : ',traceback.format_exc())
        return traceback.format_exc()
...
your_fin_analysis_sub_comment = ['Financial Metrics','Net Worth Projection','Liability Analysis']
your_fw_plan_idx = 0 
...
def content(pdf,json_data,c_MoneyS,money_signData):
    ...
    cont_headings = ['Your Financial Profile','Your Financial Analysis','Financial Products Featured List','Best Practices']
    cont_para = ["Financial Behaviour Score, Your 1 view, Assets, Liabilities", "Financial Metrics", "Term Insurance Plans, Health Insurance Plans, Equity Mutual Funds, Debt Mutual Funds, Hybrid Mutual Funds, Credit Cards, Housing Lenders","Building a Strong Credit Profile, Planning Your Income Taxes, Available Tax Deductions, Capital Gains Taxation by Asset Type, Planning For Inheritance, Understanding Inheritance’s Tax Implications, Planning Your Estate and Will"] 
    index_no = [your_fin_prof_idx,your_fin_analysis_idx,fin_feat_product_list,best_practices_idx]
...
def liabilities_chart(pdf,json_data,c_MoneyS,money_signData):
    ...
    def get_rect_h(pdf,df_table,y):
        ...
        pdf.set_text_color(*hex2RGB('#1A1A1D'))
        pdf.set_xy(px2MM(710),px2MM(state_y)) 
        pdf.multi_cell(px2MM(257), px2MM(32),str(df_table['liability']),align='L')
    ...
    for i in range(len(df_table)):
        ...
        if df_table[i]['interest_rate'] == 0 or df_table[i]['interest_rate'] =='':
            ...
            pdf.cell(px2MM(100), px2MM(32),str(round(df_table[i]['interest_rate'],1))+'%',align='R')    
...
def mf_holding_eveluation(pdf,json_data,c_MoneyS,money_signData):
    ...
    if 1080-rem1 >  (156+(cnt*42)):
        ...
        if "Mutual Fund Commission Analyser" in mf_comment2[0]:
            ...
            pdf.cell(px2MM(265), px2MM(42),"""Commission Analyser""",align='L',link='https://1finance.co.in/calculator/mutual-funds')
            x = mm2PX(pdf.get_x())
            pdf.set_text_color(*hex2RGB('#000000'))
            pdf.set_font('LeagueSpartan-Regular',size=px2pts(30))
        ...
        #//*---Point 2
        start_point2 = mf_comment2[1].split('Mutual Fund Scoring and Ranking')
        pdf.set_font('LeagueSpartan-Regular', size=px2pts(30))
        pdf.set_text_color(*hex2RGB('#000000'))
        pdf.set_fill_color(*hex2RGB('#000000'))
        ...
        pdf.set_xy(px2MM(142),px2MM(rem2+94))
        ...
        pdf.set_xy(px2MM(142),px2MM(rem2+94))
        pdf.cell(px2MM(350), px2MM(42),"""Scoring and Ranking page.""",align='L',link='https://1finance.co.in/product-scoring/mutual-funds?type=equity')
...
def cashflow_plan(pdf,json_data,c_MoneyS,money_signData):
    ...
    featured_list = json_data['next_three_months_action_plan']['cashflow_comments']
    if featured_list == []:
        return None
...
def next_quarter_preview(pdf, json_data):
    ...
    def next_quarter_preview_pg_setup(pdf, date):  
        ...
        pdf.set_font('LeagueSpartan-SemiBold', size=px2pts(60))
        pdf.set_text_color(*hex2RGB('#1A1A1D'))
        pdf.cell(px2MM(792), px2MM(84), "Preview for Next Meeting")
        ...
        pdf.set_font('LeagueSpartan-Regular', size=px2pts(40))
        pdf.set_text_color(*hex2RGB('#65676D'))
        pdf.cell(px2MM(420), px2MM(60), "Next Review meeting date")
...
def disclaimer(pdf,json_data,c_MoneyS,money_signData):
    ...
    pdf.multi_cell(px2MM(350), px2MM(32),'BASL Registration No :',border=0,align="L")
    ...
    pdf.set_xy(px2MM(357),px2MM(1008))  
